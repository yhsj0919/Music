...
package xyz.yhsj.mediabrowser.service.notification;
...
import xyz.yhsj.mediabrowser.service.utils.BitmapUtil;
...
public final class AlbumArtCache {
    ...
    private static final int MAX_ALBUM_ART_CACHE_SIZE = 10 * 1024 * 1024;  // 12 MB
    private static final int MAX_ART_WIDTH = 300;  // pixels
    private static final int MAX_ART_HEIGHT = 300;  // pixels
...
//    }
    ...
    public void fetch(final String artUrl, final FetchListener listener) {
        ...
        if (bitmap != null) {
            ...
            listener.onFetched(artUrl, bitmap[BIG_BITMAP_INDEX], bitmap[BIG_BITMAP_INDEX]);
            return;
        }
        ...
        new AsyncTask<Void, Void, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(Void[] objects) {
                Bitmap[] bitmaps;
                try {
                    Bitmap bitmap = BitmapUtil.fetchAndRescaleBitmap(artUrl,
                            MAX_ART_WIDTH, MAX_ART_HEIGHT);
                    if (bitmap == null)
                        return null;
                    Bitmap icon = BitmapUtil.scaleBitmap(bitmap,
            ...
            protected void onPostExecute(Bitmap[] bitmaps) {
                if (bitmaps == null) {
                    ...
                    listener.onFetched(artUrl,
                            bitmaps[BIG_BITMAP_INDEX], bitmaps[BIG_BITMAP_INDEX]);
                }
            }
        }.execute();
    }
